...
2. **优先后端 STT**：约 2.8s 短 WAV 循环 → `POST /api/speech/stt` → 短语匹配（静音块会跳过上传）  
3. 后端不可用或连续失败时 **自动回退** 设备 `SpeechRecognizer`  
4. 支持「唤醒 + 指令」一句话：`小爪醒醒，今天怎么样`  
5. 也可先唤醒，再单独说下一句  
6. 进入免提会话后可持续多轮，直到超时或说停止词（`再见` / `结束` / `bye` 等）  
7. 点击麦克风手动说话时，会暂时暂停唤醒监听；对话/TTS 结束后自动恢复  

徽章在唤醒模式下显示 `STT:WAKE/FUNASR`（或 `WAKE/DEVICE`）。
